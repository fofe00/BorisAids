    ...
    <div class="lg:w-1/2 xl:max-w-screen-sm">
        <div class="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
            <div class="cursor-pointer flex items-center">
                <div class="text-2xl text-gray-800 tracking-wide ml-2 font-semibold">borisAids</div>
            </div>
        </div>
            ...
            <div class="mt-12">
                <form method="post" action="{{ route('traitement') }}">
                    ...
                    <div>
                        ...
                        <input class="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type="email" name="email" value="{{ old('email') }}" placeholder="mike@gmail.com">
                    </div>
                    @if($errors->has('email'))
                        <p class="text-lg text-red-600">{{ $errors->first('email') }}</p>
                    @endif
                    <div class="mt-8">
                    ...
                    @if($errors->has('password'))
                        <p class="text-lg text-red-600">{{ $errors->first('password') }}</p>
                    @endif
                    <div class="mt-10">
                        <button class="bg-gray-800 text-gray-100 p-4 w-full rounded tracking-wide
                ...
                <div class="mt-12 text-sm font-display font-semibold text-gray-700 text-center">
                    Don't have an account ? <a class="cursor-pointer text-gray-600 hover:text-gray-800">Sign up</a>
                </div>
            </div>
        </div>
    </div>
